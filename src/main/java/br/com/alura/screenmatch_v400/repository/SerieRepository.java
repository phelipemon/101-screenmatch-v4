...
public interface SerieRepository extends JpaRepository<Serie, Long> {
    ...
    List<Episodios> obterEpisodioPorTemporada(Long id, Long numero);

    @Query("SELECT e FROM Serie s JOIN s.episodios e WHERE s.id = :id ORDER BY e.avaliacao DESC LIMIT 5")
    List<Episodios> obterTop5EpisodiosPorSerie(Long id);
}
